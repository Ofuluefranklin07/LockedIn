...
export default function AICoach() {
  ...
  const generateFeedback = async () => {
    ...
    try {
      const [logsResult, goalsResult] = await Promise.allSettled([
        getDocs(query(
      ...
      if (logsResult.status === 'rejected') {
        console.warn('AI insights could not read daily logs:', logsResult.reason);
      }

      if (goalsResult.status === 'rejected') {
        console.warn('AI insights could not read goals:', goalsResult.reason);
      }

      const logs = logsResult.status === 'fulfilled'
        ? logsResult.value.docs
          .map((d) => ({ id: d.id, ...d.data() }) as DailyLog)
          .filter((log) => typeof log.date === 'string')
          .sort((a, b) => b.date.localeCompare(a.date))
          .slice(0, 14)
        : [];
      const goals = goalsResult.status === 'fulfilled'
        ? goalsResult.value.docs.map((d) => ({ id: d.id, ...d.data() }) as Goal)
        : [];
  ...
  useEffect(() => {
    if (!hasAutoGenerated && user && profile) {
      setHasAutoGenerated(true);
      generateFeedback();
    }
  }, [hasAutoGenerated, profile, user]);
